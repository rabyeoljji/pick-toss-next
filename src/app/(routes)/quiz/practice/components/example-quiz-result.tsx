...
interface Props {
  ...
  results: ExampleSolvingData
}

export default function ExampleQuizResult({ totalElapsedTime, results }: Props) {
  const router = useRouter()

  const quizTypeDetailResults = useMemo(() => {
    const { quizTypeMap, totalCorrect, totalQuestions } = results.reduce(
      (acc, result) => {
        const quizTypeName = result.quizType === 'MIX_UP' ? 'O/X 퀴즈' : '객관식 퀴즈'

        if (!acc.quizTypeMap[quizTypeName]) {
          acc.quizTypeMap[quizTypeName] = {
            name: quizTypeName,
            correct: 0,
            total: 0,
          }
        ...
        acc.quizTypeMap[quizTypeName].total += 1
        if (result.answer) {
          acc.quizTypeMap[quizTypeName].correct += 1
          acc.totalCorrect += 1
        }
      ...
      {
        quizTypeMap: {} as { [key: string]: { name: string; correct: number; total: number } },
        totalCorrect: 0,
        totalQuestions: 0,
      }
    )

    const details = Object.values(quizTypeMap)
    ...
    <div className="mt-[45px] flex flex-col items-center gap-[24px] pb-[86px]">
      <div className="flex h-[58px] flex-col items-center justify-end gap-[8px]">
        ...
        <div className="relative flex h-[30.76px] items-end">
          <div className="text-h3-bold text-gray-08">연습 퀴즈 완료!</div>
      ...
      <div className="w-[335px] rounded-[16px] bg-white pb-[16px] pt-[45px]">
        ...
        >
          <div className="flex flex-col items-center gap-[8px]">
            <BadgeIcon />
            <div className="text-h1 text-gray-08">{quizTypeDetailResults.score || 0}점</div>
          ...
          <div className="mb-[43px] flex flex-col gap-[24px] px-[24px]">
            <div className="text-body1-bold text-gray-08">퀴즈 유형별 상세</div>
            <div className="flex flex-col gap-[16px]">
              {quizTypeDetailResults.details.map((detail) => (
          ...
          <div className="px-[12px]">
            <Button className="w-full" onClick={() => router.replace('/main')}>
              확인
            </Button>
          </div>
        </motion.div>
